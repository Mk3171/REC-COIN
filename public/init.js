...
function calcOfflineEarnings() {
  try {
    ...
    // ✅ Apply earnings DIRECTLY to rec (no claim needed)
    record += earnedRecord;
    rec += earnedRec;

    saveData(true);

    // Show info popup (already added to balance)
    ...
    ol.addEventListener('click', function(){ ol.remove(); });
    ...
    pp.innerHTML =
      ...
      (earnedRecord > 0 ? '<div style="background:rgba(255,0,0,0.08);border:1px solid rgba(255,0,0,0.2);border-radius:10px;padding:10px;margin-bottom:8px;"><div style="font-size:11px;color:#aaa;margin-bottom:3px;">⚡ RECORD Earned</div><div style="font-size:18px;color:#FF6644;font-family:Orbitron,sans-serif;font-weight:bold;">+'+formatCost(earnedRecord)+'</div></div>' : '')+
      (earnedRec > 0.000001 ? '<div style="background:rgba(0,255,136,0.06);border:1px solid rgba(0,255,136,0.2);border-radius:10px;padding:10px;margin-bottom:14px;"><div style="font-size:11px;color:#aaa;margin-bottom:3px;">🟢 REC Earned</div><div style="font-size:18px;color:#00FF88;font-family:Orbitron,sans-serif;font-weight:bold;">+'+earnedRec.toFixed(6)+'</div></div>' : '<div style="margin-bottom:14px;"></div>')+
      '<div style="font-size:11px;color:#00FF88;margin-bottom:10px;">✅ Added to your balance automatically</div>'+
      '<button onclick="this.closest(\'div\').parentElement.remove()" style="background:#1a1a1a;border:1px solid #333;color:#888;padding:10px;border-radius:10px;cursor:pointer;font-size:13px;width:100%;">Close ✕</button>';
    ol.appendChild(pp);
